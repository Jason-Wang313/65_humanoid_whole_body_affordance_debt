        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Humanoid Whole-Body Affordance Debt: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study humanoid whole-body manipulation. The motivating bet is: Measure and minimize future affordance loss induced by current whole-body posture choices.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For humanoid whole-body manipulation, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
proposed\_branch\_mechanism & nominal & 0.800 & 0.200 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Humanoid Whole-Body Affordance Debt argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
